...
Where s.Classification = 'Freshman' AND s.MentorID = i.InstructorID AND i.InstructorID = p.ID;
...
WHERE i.InstructorID NOT IN (o.InstructorID);
...
/* FROM Instructor i, Person p, Offering o, Student s */
/* WHERE i.InstructorID = p.ID AND i.InstructorID <> o.InstructorID  AND i.InstructorID <> s.MentorID; */

/* Item 19. Find the IDs, names and DOB of the youngest student(s). */
SELECT ID, Name, DOB
FROM Person p, Student s
WHERE p.ID = s.StudentID AND (DOB=(SELECT MIN(p.DOB) FROM Person)) ORDER BY DOB DESC;

/* Item 20. List the IDs, DOB, and Names of Persons who are neither a student
nor a instructor. */
/* TODO: 1000 ROWS Of Wee Fletcher */
SELECT ID,DOB,Name
FROM Person p, Student s, Instructor i
WHERE p.ID NOT IN (SELECT StudentID FROM Student s2) OR p.ID NOT IN (SELECT InstructorID FROM Instructor i2);

/* Item 21. For each instructor list his / her name and the number of
students he / she mentors. */
SELECT Name, COUNT(*)
FROM Instructor i
    INNER JOIN Person AS InstructorInfo
        ON InstructorInfo.ID = i.InstructorID
